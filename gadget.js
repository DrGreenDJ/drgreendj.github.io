...
const store = /* @__PURE__ */ defineStore("store", {
  state: () => ({
    conversationUid: void 0,
    fromAddress: "",
    agentEmail: "",
    queueName: ""
  }),
  actions: {
    async setConversationUid(uid2) {
      this.conversationUid = uid2;
    },
    async setFromAddress(address) {
      this.fromAddress = address;
    },
    async setAgentEmail(email) {
      this.agentEmail = email;
    },
    async setQueueName(name) {
      this.queueName = name;
    }
  }
});
  ...
  setup(__props) {
    ...
    function _setupEventListeners() {
      distExports.Desktop.agentContact.addEventListener(
        "eAgentContact",
        (msg) => console.error("NTS DEBUG: AgentContact eAgentContact: ", msg)
      );
      distExports.Desktop.agentContact.addEventListener(
        ...
        (contact) => {
          ...
          try {
            ...
            for (const participantId in Object.keys(participants)) {
              if (participantId != agentId) {
                searchCustomer(participantId);
              }
            }
            storeData.setQueueName(contact.data.interaction.callProcessingDetails.virtualTeamName);
          } catch (ex) {
    ...
    function searchCustomer(fromAddress) {
      if (!props.apiKey) {
        console.error("Unable to call merkur API without key");
        return;
      }
      storeData.setFromAddress(fromAddress);
      const url = "https://contactcenter.atliwest.local/rest/v1/merkurbooster";
      const config = {
        headers: {
      ...
      try {
        axios.get(`${url}/customer/phone/${fromAddress}/identify`, config).then((response) => {
          console.error("NTS DEBUG: customer data", response);
          storeData.setConversationUid(response.data.conversation_uid);
    ...
    onMounted(() => {
      console.error("NTS DEBUG: mounted");
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("main", null, [
        _cache[0] || (_cache[0] = createBaseVNode("label", null, "This is a placeholder, it's nothing special but it's mine", -1)),
...
const App = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-4423d42e"]]);
