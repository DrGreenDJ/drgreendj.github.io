...
class StoredData {
  static get apiUrl() {
    return "https://contactcenter.atliwest.local/rest/v1/merkurbooster";
  }
  ...
  static addInteraction(conversationId, fromAddress, queueName) {
    const newInteraction = new InteractionData(conversationId, fromAddress, queueName);
    this._conversationMap.set(conversationId, newInteraction);
    return newInteraction;
  }
  static getInteraction(conversationId) {
    if (!this._conversationMap.has(conversationId)) {
      return null;
    }
    return this._conversationMap.get(conversationId);
  }
  static removeInteraction(interactionId) {
    this._conversationMap.delete(interactionId);
  }
...
__publicField(StoredData, "queueName", "");
__publicField(StoredData, "_conversationMap", /* @__PURE__ */ new Map());
__publicField(StoredData, "_apiKey");
class InteractionData {
  constructor(_interactionId, _fromAddress, _queueName) {
    __publicField(this, "_conversationUid");
  ...
  get conversationUid() {
    return this._conversationUid;
  }
  setConversationUid(value) {
    this._conversationUid = value;
  }
}
const _hoisted_1 = { class: "ml-2 mr-2 mt-3 mb-3" };
const _hoisted_2 = { class: "row" };
const _hoisted_3 = { class: "col-12 mt-3" };
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    const customerSearchResult = ref(/* @__PURE__ */ new Map());
    const search = ref("");
    watch(search, () => {
      if (search.value.length < 3) return;
      const amendedCustomerDict = /* @__PURE__ */ new Map();
      axios.get(`${StoredData.apiUrl}/customer/search/all/${search.value}?limit=20`, StoredData.apiConfig).then((result) => {
        console.error("customer search result", result.data);
        try {
          const custData = result.data.data;
          ...
          customerSearchResult.value = amendedCustomerDict;
        } catch (ex) {
          console.error("Error while parsing customer search result", ex);
        }
      }).catch((reason) => {
            ...
            createBaseVNode("table", _hoisted_6, [
              _cache[3] || (_cache[3] = createBaseVNode("thead", null, [
                createBaseVNode("tr", null, [
                  createBaseVNode("th", { style: { "width": "125px" } }, "PLZ"),
                  createBaseVNode("th", null, "Kundendaten")
                ])
              ], -1)),
                    ...
                    createBaseVNode("td", null, [
                      createBaseVNode("table", _hoisted_7, [
                        _cache[2] || (_cache[2] = createBaseVNode("thead", null, [
                          createBaseVNode("tr", null, [
                            createBaseVNode("th", null, "Kundennummer"),
                            createBaseVNode("th", null, "Name"),
                            createBaseVNode("th", null, "Vertragsadresse"),
                            createBaseVNode("th", null, "Rechnungsadresse"),
                            createBaseVNode("th", null, "Kunde wählen")
                          ])
                        ], -1)),
                        createBaseVNode("tbody", null, [
                          (openBlock(true), createElementBlock(Fragment, null, renderList(customerSearchResult.value.get(zipCode), (customer) => {
                            return openBlock(), createElementBlock("tr", {
                              ...
                              createBaseVNode("td", null, toDisplayString(customer.debitor_no), 1),
                              createBaseVNode("td", null, toDisplayString(customer.customer_name), 1),
                              createBaseVNode("td", null, toDisplayString(customer.contract_address), 1),
                              createBaseVNode("td", null, toDisplayString(customer.invoice_address), 1),
                              createBaseVNode("td", null, [
                                createBaseVNode("div", {
                                  class: "btn btn-sm btn-success",
                                  onClick: ($event) => selectCustomer(customer)
                                }, " Auswählen ", 8, _hoisted_8)
                              ])
                            ]);
                          }), 128))
                        ])
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    if (props.apiKey) StoredData.setApiKey(props.apiKey);
    else console.error("Unable to request customer data without apiKey!");
    try {
    ...
    function test2() {
      console.error("NTS DEBUG: Attempting to get latest data");
      try {
        const latestData = distExports.Desktop.agentStateInfo.latestData;
        console.error("NTS DEBUG: Got data", latestData);
      } catch (ex) {
        console.error("NTS DEBUG: Error while getting data", ex);
      }
      console.error("NTS DEBUG: Finished getting data");
      try {
        console.error(props);
      } catch (ex) {
        console.error("Error while reading props", ex);
      }
    }
    function _setupEventListeners() {
          ...
          try {
            ...
            for (const participantId of Object.keys(participants)) {
              if (participantId != agentId) {
                ...
                searchCustomer(participantId, contact.data.interactionId);
              }
            }
          } catch (ex) {
      ...
      distExports.Desktop.agentContact.addEventListener(
        ...
        (contact) => {
          console.error("NTS DEBUG: AgentContact eAgentContactAssigned: ", contact);
        }
      );
      distExports.Desktop.agentContact.addEventListener("eAgentContactEnded", (msg) => {
        var _a;
        console.error("NTS DEBUG: AgentContact eAgentContactEnded: ", msg);
        StoredData.removeInteraction(msg.data.interactionId);
        if (((_a = currentInteraction.value) == null ? void 0 : _a.interactionId) === msg.data.interactionId) {
          currentInteraction.value = void 0;
        }
      });
    }
    function searchCustomer(fromAddress, interactionId) {
      if (!props.apiKey) {
        console.error("Unable to call merkur API without key");
        return;
      }
      try {
        axios.get(`${StoredData.apiUrl}/customer/phone/${fromAddress}/identify`, StoredData.apiConfig).then((response) => {
          console.error("NTS DEBUG: customer data", response);
          const interaction = StoredData.getInteraction(interactionId);
          if (interaction) {
            interaction.setConversationUid(response.data.conversation_uid);
          }
        }).catch((reason) => console.error("Search api error", reason));
      } catch (ex) {
        console.error("Error while searching customer", ex);
      }
    }
    return (_ctx, _cache) => {
      var _a;
      return openBlock(), createElementBlock("main", null, [
        createBaseVNode("button", { onClick: test2 }, "Get latest agent info"),
        createVNode(_sfc_main$1, {
          "interaction-id": (_a = currentInteraction.value) == null ? void 0 : _a.interactionId
        }, null, 8, ["interaction-id"])
      ]);
    };
  }
});
...
const App = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-80feb369"]]);
